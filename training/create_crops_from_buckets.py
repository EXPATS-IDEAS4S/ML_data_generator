...
# Set this to True if 6.2-10.8 difference should be used 
OT = True 

if OT:
    value_fields = [value_max[0], -60] 
else:
    value_fields = [value_max[0], value_max[1]]
    ...
    for month in months:
        ...
        for day in days:
            ...
            if my_obj is not None:
                ...
                #     ds_day_var = apply_cma_mask(ds_day, ds_day_var, value_max[0])	#TODO make var and velue max in a dictionary
                ...
                for timestamp in timestamps:
                    ...
                    if not is_all_nan_ds and not is_outside_range and month >= month_start and month <= month_end and hour >= hour_start and hour < hour_end:          
                        print(f"Processing file: {file} for timestamp: {timestamp}")
                        ...
                        if apply_cma and 'cma' in ds_time and 'IR_108' in cloud_prm:
                        ...
                        if cropping_strategy == 'random':
                            crops_nc_random(ds_time_var, x_pixel, y_pixel, n_samples, filename_to_save, outpath, file_extension)
                        elif cropping_strategy == 'fixed':
                            #print('fixed')
                            crops_nc_fixed(ds_time_var, x_pixel, y_pixel, [(crop_ul_lat,crop_ul_lon)], filename_to_save, outpath, file_extension)         
                        else:
